...
for ($i=0;$i<count($tab_props);$i++){
	...
	/*$sql="SELECT label_page.id, p$prop.nb, p$prop.nbimg FROM label_page, p$prop WHERE  label_page.id_art_or_prop=p$prop.id AND label_page.prop=$prop";// GROUP BY label_page.prop, label_page.qwd, label_page.lg";
	$rep=mysqli_query($link,$sql);
	while($data = mysqli_fetch_assoc($rep)) {
		$sql="UPDATE label_page SET nb=".$data["nb"].", nbimg=".$data["nbimg"]." WHERE id=".$data["id"];
		mysqli_query($link,$sql);
	}*/
}
