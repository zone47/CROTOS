...
while($data = mysqli_fetch_assoc($rep)) {
	...
	$where.=")";
	$sql="select commons_img.P18,commons_img.width,commons_img.height from artw_prop,artworks, commons_img WHERE ".$where." AND artw_prop.prop=".$prop." AND artw_prop.id_artw=artworks.id AND artworks.P18=commons_img.id";
	...
	$facet="<a href=\"../?p$prop=$qwd";
	if ($mode==1)
		$facet.= "&c18=1";
	$facet.= "\" class=\"facet item\" style=\"background-image:url('".$img."')\"><div class=\"bandeau\"><div>$lb<br/><span class=\"nbimg\">$nbimg <span class=\"nbres\">";
	if ($nbimg<2)
		$facet.=translate($l,"result");
	else
		$facet.=translate($l,"results");
	$facet.="</span></span></div></div></a>\n";
	echo $facet;
}
?>
</div>