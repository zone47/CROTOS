...
while($data = mysqli_fetch_assoc($rep)) {
	...
	$lb=truncate(label_item($qwd,$l),60);
	//echo "+++".$qwd."+++";
	//echo "+++".$lb."+++";
	...
	if ($mode==1)
		$link_facet.= "&c18=1";
	$facet="<div onClick=\"window.location.href='".$link_facet."'\" class=\"facet item\" style=\"background-image:url('".$img."')\" ><div class=\"bandeau";
	if ($prop==608)
		$facet.=" bandeau608";
	$facet.="\"><div><a href=\"".$link_facet."\" class=\"link_facet\">$lb</a>";
	
	if ($prop==608){
		$facet.="<p>";
		if ($mbegin!=0)
			$facet.=translate($l,"m".$mbegin)." ";
		if ($ybegin!=0)
			$facet.=$ybegin." - ";
		if ($mend!=0)
			$facet.=translate($l,"m".$mend)." ";
		if ($yend!=0)
			$facet.=$yend." ";
		$facet.="<br/>";
		if ($explink!="")
			$facet.="<a href=\"".$explink."\" class=\"expolink \">".substr($explink,0,52)."...</a>";
		$facet.="</p>";
	}else
		$facet.="<br/>";
	$facet.="<span class=\"nbimg\">";
	$facet.="$nbimg <span class=\"nbres\">";
	if ($nbimg<2)
