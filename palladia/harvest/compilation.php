...
mysqli_query($link,"TRUNCATE `p189`");
...
mysqli_query($link,"TRUNCATE `p1071`");
mysqli_query($link,"TRUNCATE `p1433`");
mysqli_query($link,"TRUNCATE `p1639`");
mysqli_query($link,"TRUNCATE `p1684`");
mysqli_query($link,"TRUNCATE `p2079`");
mysqli_query($link,"TRUNCATE `p2596`");
mysqli_query($link,"TRUNCATE `p4896`");
...
while($file = readdir($dir)) {
	...
	if($file != '.' && $file != '..' && !is_dir($dirname.$file)){
		...
		$tab_miss = array(
			...
			"m170"=> 0,// creator
			"m186"=> 0,// material
			"m189"=> 0,// location of discovery
			"m195"=> 0,// collection
			"m276"=> 0,// location
			"m361"=> 0,// part of
			"m921"=> 0,// subject heading
			"m941"=> 0,// inspired by
			"m1071"=> 0,// location of creation
			"m1684"=> 0,// Inscription
			"m2079"=> 0,// fabrication methody
			"m2596"=> 0,// culture
			"m4896"=> 0,// 3D model
			"m6216"=> 0,// copyright status
...
if ($offic_url=="")
	$offic_url=urlext_search_MSR($item,$claims);
...
// Other properties
$tab_multi=array(31,135,136,144,170,179,180,186,189,195,276,361,608,921,941,1071,1433,1639,2079,2596,6216);	
for ($i=0;$i<count($tab_multi);$i++){
	if ($claims["P".$tab_multi[$i]])
		foreach ($claims["P".$tab_multi[$i]] as $value){
			...
			$attrib=false;
			if ($tab_multi[$i]==170){
				if(intval($value["mainsnak"]["datavalue"]["value"]["numeric-id"]==4233718))
					$alive=false;
				if (isset($value["qualifiers"]["P1480"]))
					foreach ($value["qualifiers"]["P1480"] as $value_sys){
						if($value_sys["datavalue"]["value"]["numeric-id"]==18122778);
							$attrib=true;
				}
				if (isset($value["qualifiers"]["P5102"]))
					foreach ($value["qualifiers"]["P5102"] as $value_sys){
						if($value_sys["datavalue"]["value"]["numeric-id"]==230768);
							$attrib=true;
				}
			}
			if ($tab_multi[$i]==608){
				$alive=false;
				if(test_expoMSR(intval($value["mainsnak"]["datavalue"]["value"]["numeric-id"])))
					$alive=true;
			}
			...
			if ($alive){		
				...
				$insertok=true;
				if (($tab_multi[$i]==195)||($tab_multi[$i]==276)||($tab_multi[$i]==189)||($tab_multi[$i]==1071)){
					...
					if (((!$found)||($level!=0))&&($tab_multi[$i]!=189))
				...
				if ($insertok){
					...
					$rep=mysqli_query($link,$sql);
					if ($attrib){
						$sql="SELECT max(id) as maxid FROM artw_prop";
						$rep=mysqli_query($link,$sql);
						$row = mysqli_fetch_assoc($rep);
						$sql="INSERT INTO q170 (idaff,value) VALUES (".$row['maxid'].",1)";
						$rep=mysqli_query($link,$sql);
					}
				}
			}
...
$tab_others=array(1684,4896);	
for ($i=0;$i<count($tab_others);$i++){
	if ($claims["P".$tab_others[$i]]){
		foreach ($claims["P".$tab_others[$i]] as $value){
			if ($tab_others[$i]==1684){
				$val=$value["mainsnak"]["datavalue"]["value"]["text"];
				$lg=$value["mainsnak"]["datavalue"]["value"]["language"];
				$sql="INSERT INTO p".$tab_others[$i]." (text,lang) VALUES (\"".$val."\",\"".$lg."\")";
				$rep=mysqli_query($link,$sql);
			}elseif ($tab_others[$i]==4896){
				$val=$value["mainsnak"]["datavalue"]["value"];
				$sql="INSERT INTO p".$tab_others[$i]." (link) VALUES (\"".$val."\")";
				$rep=mysqli_query($link,$sql);
			}
			$sql="SELECT max(id) as maxid FROM p".$tab_others[$i];
			$rep=mysqli_query($link,$sql);
			$row = mysqli_fetch_assoc($rep);
			$sql="INSERT INTO artw_prop (prop,id_artw,id_prop) VALUES (".$tab_others[$i].",".$id_artwork.",".$row['maxid'].")";
			$rep=mysqli_query($link,$sql);
		}
	}
	else
		$tab_miss["m".$tab_others[$i]]=1;
}
